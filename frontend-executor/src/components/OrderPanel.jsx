  ...
  return (
    <>
      <div className="exec-sidebar-section">
        <div className="exec-sidebar-title">{t("挂单")} ({pendingOrders.length})</div>
        {pendingOrders.length === 0 ? (
          <div style={{ color: "var(--exec-text-secondary)", fontSize: 12, padding: "8px 0" }}>
            <div>{t("暂无挂单")}</div>
            <div style={{ marginTop: 4, opacity: 0.7 }}>{t("策略部署到执行器后将在此显示订单状态。")}</div>
          </div>
        ) : (
          pendingOrders.map(o => (
            <div key={o.order_id} className={`exec-order-row ${o.side === "Buy" ? "buy" : "sell"}`}>
              <span>{o.symbol || "?"}</span>
              <span>{o.side === "Buy" ? t("买入") : t("卖出")}</span>
              <span>{o.quantity || 0}</span>
              <span className="exec-order-status">{orderStatusText(o.status, t)}</span>
            </div>
          ))
        )}
      </div>
      <div className="exec-sidebar-section">
        <div className="exec-sidebar-title">{t("成交记录")} ({filledOrders.length})</div>
...
function orderStatusText(status, t) {
  switch (status) {
    case "Created": return t("已创建");
    case "Submitted": return t("已提交");
    case "Accepted": return t("已接受");
    case "PartiallyFilled": return t("部分成交");
    case "Filled": return t("已成交");
