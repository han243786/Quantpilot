...
mod tests {
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_unknown_transition_event() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            .find(|machine| machine.machine_id == "intent.trend")
            .unwrap();
        intent.transitions[0].event.event_type = "graph.intent.missing".to_string();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_unknown_event_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::EventPayload,
                path: "symbol".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `intent.trend`")
                ...
                && message.contains("event_type `graph.intent.missing`")
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_missing_event_catalog() {
        let mut graph = sample_machine_graph();
        ...
        let intent = graph
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_missing_catalog_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::EventPayload,
                path: "symbol".to_string(),
            }],
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_unknown_readonly_runtime_fact() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_unknown_runtime_fact_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::ReadonlyRuntimeFact,
                path: "provider.secret".to_string(),
            }],
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_unknown_runtime_fact_guard`")
                && message.contains("reads unknown readonly runtime fact `provider.secret`")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_forbidden_parameter_path() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_forbidden_parameter_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: vec!["graph.edges".to_string()],
            conditions: Vec::new(),
            policy: None,
            explanation: None,
        });
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_forbidden_parameter_guard`")
                && message.contains("parameter path `graph.edges`")
                && message.contains("outside the proposal-only guard boundary")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_invalid_condition_operand() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_invalid_condition_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_invalid_condition_guard`")
                && message.contains("condition `missing_parameter`")
                && message.contains("references undeclared parameter path `guard.missing`")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_duplicate_condition_id() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_duplicate_condition_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
            ...
            conditions: vec![
                MachineGuardConditionSpec {
                    condition_id: "repeat_condition".to_string(),
                    left_read: MachineGuardReadRef {
                        source: MachineGuardReadSource::MachineMemory,
                        path: "last_signal_at".to_string(),
                    },
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_duplicate_condition_guard`")
                && message.contains("declares duplicate condition `repeat_condition`")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_condition_empty_left_read_path() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_condition_empty_left_read_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
            parameter_paths: vec!["guard.threshold".to_string()],
            conditions: vec![MachineGuardConditionSpec {
                condition_id: "empty_condition_left_read".to_string(),
                left_read: MachineGuardReadRef {
                    source: MachineGuardReadSource::MachineMemory,
                    path: "".to_string(),
                },
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_condition_empty_left_read_guard`")
                && message.contains("condition `empty_condition_left_read`")
                && message.contains("has an empty left read path")
        }));
    }

    #[test]
    fn machine_graph_rejects_guard_descriptor_condition_empty_right_parameter_path() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_condition_empty_right_parameter_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_condition_empty_right_parameter_guard`")
    ...
    #[test]
    fn machine_graph_rejects_guard_descriptor_without_reads() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_empty_surface_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: Vec::new(),
            conditions: Vec::new(),
            policy: None,
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_empty_surface_guard`")
                && message.contains("must declare at least one read")
        }));
    }

    #[test]
    fn machine_graph_rejects_guard_descriptor_invalid_policy() {
        let mut graph = sample_machine_graph();
        let intent = graph
            .machines
            .iter_mut()
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "graph_invalid_policy_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = graph.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `graph_invalid_policy_guard`")
    ...
    #[test]
    fn machine_graph_rejects_child_guard_descriptor_without_reads() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_no_read_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: Vec::new(),
            conditions: Vec::new(),
            policy: None,
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `child_graph_no_read_guard`")
                && message.contains("must declare at least one read")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_transition_action_undeclared_memory_write() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = Some(MachineActionSpec {
            emits: Vec::new(),
            memory_writes: vec!["unscoped_child_write".to_string()],
            diagnostics: vec!["child_write_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                ...
                && message.contains("writes undeclared memory field `unscoped_child_write`")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_transition_action_empty_event_emit() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = Some(MachineActionSpec {
            ...
            diagnostics: vec!["child_empty_emit_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard.child`")
                && message.contains("transition `risk.guard.child.transition`")
                && message.contains("action emits an empty event_type")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_transition_action_unknown_event_emit() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].guard = None;
        child.transitions[0].action = Some(MachineActionSpec {
            emits: vec!["risk.child.emit.missing".to_string()],
            memory_writes: Vec::new(),
            diagnostics: vec!["child_unknown_emit_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard.child`")
                ...
                && message.contains("event_type `risk.child.emit.missing`")
                && message.contains("must be declared in event_catalog")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_transition_action_unauthorized_event_emit() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].guard = None;
        child.transitions[0].action = Some(MachineActionSpec {
            emits: vec!["risk.child.emit.denied".to_string()],
            memory_writes: Vec::new(),
            diagnostics: vec!["child_unauthorized_emit_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard.child`")
                && message.contains("transition `risk.guard.child.transition`")
                && message.contains("cannot emit event `risk.child.emit.denied`")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_child_transition_unknown_to_state() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].event.event_type = "risk.child.check".to_string();
        child.transitions[0].event.source = Some("risk.guard".to_string());
        child.transitions[0].to_state = "risk.child.missing".to_string();
        child.transitions[0].guard = None;
        child.transitions[0].action = None;
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains(
                    "transition `risk.guard.child.transition` references unknown to_state",
                )
        }));
    }

    #[test]
    fn machine_graph_rejects_child_transition_unknown_from_state() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].event.event_type = "risk.child.check".to_string();
        child.transitions[0].event.source = Some("risk.guard".to_string());
        child.transitions[0].from_state = "risk.child.missing".to_string();
        child.transitions[0].guard = None;
        child.transitions[0].action = None;
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains(
                    "transition `risk.guard.child.transition` references unknown from_state",
                )
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_template_mismatch() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("child_machine `risk.guard.child`")
                && message.contains("template must match parent machine template")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_parent_id_reuse() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("child_machine `risk.guard`")
                && message.contains("must not reuse parent machine_id")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_top_level_id_collision() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        assert!(errors.iter().any(|message| {
            message.contains(
                "duplicate machine `execution.router` across top-level and nested machines",
            )
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_child_machine_empty_id() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        let risk = graph
            ...
            .find(|machine| machine.machine_id == "risk.guard")
            .unwrap();
        let mut child = sample_machine_with("", MachineTemplateKind::Decision, risk.priority + 1);
        child.transitions[0].event.event_type = "risk.child.check".to_string();
        child.transitions[0].event.source = Some("risk.guard".to_string());
        child.transitions[0].guard = None;
        child.transitions[0].action = None;
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("state `idle` child_machine must declare machine_id")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_depth_three() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
    ...
    #[test]
    fn machine_graph_rejects_child_machine_unknown_state_group() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].guard = None;
        child.transitions[0].action = None;
        child.states[0].group_id = Some("missing_child_group".to_string());
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("child_machine `risk.guard.child` failed static contract")
                && message
                    .contains("state `idle` references unknown state_group `missing_child_group`")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_duplicate_state() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].guard = None;
        child.transitions[0].action = None;
        let mut duplicate_state = child.states[0].clone();
        duplicate_state.initial = false;
        child.states.push(duplicate_state);
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("duplicate state `idle`")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_missing_initial_state() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
    ...
    #[test]
    fn machine_graph_rejects_child_machine_multiple_initial_states() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("exactly one initial state is required, found 2")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_machine_duplicate_state_group() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].guard = None;
        child.transitions[0].action = None;
        child.state_groups.push(child.state_groups[0].clone());
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("machine `risk.guard` failed static contract")
                && message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("duplicate state_group `signal_flow`")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_child_guard_descriptor_missing_condition_id() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_missing_condition_id_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `child_graph_missing_condition_id_guard`")
                && message.contains("has a condition without condition_id")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_guard_descriptor_duplicate_condition_id() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_duplicate_condition_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
            ...
            conditions: vec![
                MachineGuardConditionSpec {
                    condition_id: "repeat_child_condition".to_string(),
                    left_read: MachineGuardReadRef {
                        source: MachineGuardReadSource::MachineMemory,
                        path: "last_signal_at".to_string(),
                    },
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `child_graph_duplicate_condition_guard`")
                && message.contains("declares duplicate condition `repeat_child_condition`")
        }));
    }

    #[test]
    fn machine_graph_rejects_child_guard_descriptor_condition_undeclared_read() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_condition_undeclared_read_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
            parameter_paths: vec!["guard.threshold".to_string()],
            conditions: vec![MachineGuardConditionSpec {
                condition_id: "missing_child_condition_read".to_string(),
                left_read: MachineGuardReadRef {
                    source: MachineGuardReadSource::EventPayload,
                    path: "missing_payload".to_string(),
                },
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message
                    .contains("structured guard `child_graph_condition_undeclared_read_guard`")
                && message.contains("condition `missing_child_condition_read`")
                && message.contains("references undeclared event_payload read `missing_payload`")
        }));
    }
    ...
    #[test]
    fn machine_graph_rejects_child_guard_descriptor_condition_empty_right_parameter_path() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_condition_empty_right_parameter_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
            parameter_paths: vec!["guard.threshold".to_string()],
            conditions: vec![MachineGuardConditionSpec {
                condition_id: "empty_child_condition_right_parameter".to_string(),
                left_read: MachineGuardReadRef {
                    source: MachineGuardReadSource::MachineMemory,
                    path: "last_signal_at".to_string(),
                },
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains(
                    "structured guard `child_graph_condition_empty_right_parameter_guard`",
                )
                && message.contains("condition `empty_child_condition_right_parameter`")
                && message.contains("has an empty right parameter path")
        }));
    }
    ...
    fn machine_graph_rejects_child_guard_descriptor_invalid_policy() {
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_invalid_policy_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: Vec::new(),
            conditions: Vec::new(),
            policy: Some(MachineGuardPolicySpec {
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `child_graph_invalid_policy_guard`")
                && message.contains("timeout_ms must be greater than zero")
        }));
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
    ...
    #[test]
    fn machine_graph_rejects_child_guard_descriptor_empty_policy() {
        let mut graph = sample_machine_graph();
        graph
            .event_catalog
            .as_mut()
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "child_graph_empty_policy_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: Vec::new(),
            conditions: Vec::new(),
            policy: Some(MachineGuardPolicySpec {
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `child_graph_empty_policy_guard`")
                && message.contains("policy must declare timeout_ms, cooldown_ms, or fallback")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_unknown_event_payload_read() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_missing_payload_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::EventPayload,
                path: "missing_payload".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_missing_payload_guard`")
                && message.contains("unknown event payload field `missing_payload`")
                && message.contains("event `bar_closed`")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_event_party_violations() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let bar_closed = graph
            .event_catalog
        ...
        let intent = graph
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_event_party_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::EventPayload,
                path: "symbol".to_string(),
            }],
    ...
    fn static_contract_bundle_rejects_guard_descriptor_duplicate_inputs() {
        ...
        let intent = graph
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_duplicate_input_guard".to_string(),
            reads: vec![
                MachineGuardReadRef {
                    source: MachineGuardReadSource::MachineMemory,
                    path: "last_signal_at".to_string(),
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_duplicate_input_guard`")
                && message.contains("declares duplicate machine_memory read `last_signal_at`")
        }));
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_duplicate_input_guard`")
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_unknown_machine_memory_read() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_unknown_memory_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "unknown_memory".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_unknown_memory_guard`")
                && message.contains("reads undeclared memory field `unknown_memory`")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_base_hygiene_violations() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_forbidden_parameter_path() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_forbidden_parameter_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: vec!["graph.edges".to_string()],
            conditions: Vec::new(),
            policy: None,
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_forbidden_parameter_guard`")
                && message.contains("parameter path `graph.edges`")
                && message.contains("outside the proposal-only guard boundary")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_invalid_condition_operand() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_invalid_condition_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_invalid_condition_guard`")
                && message.contains("condition `missing_parameter`")
                && message.contains("references undeclared parameter path `guard.missing`")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_missing_condition_id() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_missing_condition_id_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_missing_condition_id_guard`")
                && message.contains("has a condition without condition_id")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_duplicate_condition_id() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_duplicate_condition_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_duplicate_condition_guard`")
                && message.contains("declares duplicate condition `repeat_condition`")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_condition_undeclared_read() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_condition_undeclared_read_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_condition_undeclared_read_guard`")
                && message.contains("condition `missing_condition_read`")
                && message.contains("references undeclared event_payload read `missing_payload`")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_condition_empty_left_read_path() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_condition_empty_left_read_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_condition_empty_left_read_guard`")
                && message.contains("condition `empty_condition_left_read`")
                && message.contains("has an empty left read path")
        }));
    }
    ...
    fn static_contract_bundle_rejects_guard_descriptor_invalid_policy() {
        ...
        let intent = graph
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_invalid_policy_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_invalid_policy_guard`")
                && message.contains("timeout_ms must be greater than zero")
        }));
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_invalid_policy_guard`")
    ...
    #[test]
    fn static_contract_bundle_rejects_guard_descriptor_empty_policy() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let intent = graph
            .machines
            ...
            .unwrap();
        intent.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_empty_policy_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: Vec::new(),
            conditions: Vec::new(),
            policy: Some(MachineGuardPolicySpec {
        ...
        let errors = bundle.validate_static_contract().unwrap_err();
        assert!(errors.iter().any(|message| {
            message.contains("structured guard `bundle_empty_policy_guard`")
                && message.contains("policy must declare timeout_ms, cooldown_ms, or fallback")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_child_guard_descriptor_missing_condition_id() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_missing_condition_id_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `bundle_child_missing_condition_id_guard`")
                && message.contains("has a condition without condition_id")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_child_guard_descriptor_duplicate_condition_id() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_duplicate_condition_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `bundle_child_duplicate_condition_guard`")
                && message.contains("declares duplicate condition `repeat_child_condition`")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_child_guard_descriptor_condition_undeclared_read() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_condition_undeclared_read_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message
                    .contains("structured guard `bundle_child_condition_undeclared_read_guard`")
                && message.contains("condition `missing_child_condition_read`")
                && message.contains("references undeclared event_payload read `missing_payload`")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_child_guard_descriptor_condition_empty_left_read_path() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_condition_empty_left_read_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message
                    .contains("structured guard `bundle_child_condition_empty_left_read_guard`")
                && message.contains("condition `empty_child_condition_left_read`")
                && message.contains("has an empty left read path")
        }));
    }
    ...
    #[test]
    fn static_contract_bundle_rejects_child_guard_descriptor_condition_empty_right_parameter_path()
    {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_condition_empty_right_parameter_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::MachineMemory,
                path: "last_signal_at".to_string(),
            }],
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains(
                    "structured guard `bundle_child_condition_empty_right_parameter_guard`",
                )
                && message.contains("condition `empty_child_condition_right_parameter`")
                && message.contains("has an empty right parameter path")
        }));
    ...
    #[test]
    fn static_contract_bundle_rejects_child_guard_descriptor_without_reads() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_no_read_guard".to_string(),
            reads: Vec::new(),
            parameter_paths: Vec::new(),
            conditions: Vec::new(),
            policy: None,
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("structured guard `bundle_child_no_read_guard`")
                && message.contains("must declare at least one read")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_child_transition_action_undeclared_memory_write() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = Some(MachineActionSpec {
            emits: Vec::new(),
            memory_writes: vec!["unscoped_bundle_child_write".to_string()],
            diagnostics: vec!["bundle_child_write_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
        ...
        assert!(errors.iter().any(|message| {
            message.contains("child_machine `risk.guard.child` failed static contract")
                && message.contains("transition `risk.guard.child.transition`")
                && message.contains("writes undeclared memory field `unscoped_bundle_child_write`")
        }));
    }

    #[test]
    fn static_contract_bundle_rejects_child_transition_action_empty_event_emit() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = Some(MachineActionSpec {
            emits: vec!["".to_string()],
            memory_writes: Vec::new(),
            diagnostics: vec!["bundle_child_empty_emit_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
    ...
    #[test]
    fn static_contract_bundle_rejects_child_transition_action_unauthorized_event_emit() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = Some(MachineActionSpec {
            emits: vec!["risk.child.emit.denied".to_string()],
            memory_writes: Vec::new(),
            diagnostics: vec!["bundle_child_unauthorized_emit_probe".to_string()],
        });
        risk.states[0].child_machine = Some(Box::new(child));
    ...
    #[test]
    fn static_contract_bundle_rejects_child_transition_without_event_type() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        let risk = graph
            .machines
    ...
    #[test]
    fn static_contract_bundle_rejects_child_transition_unknown_to_state() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_transition_unknown_from_state() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_template_mismatch() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_parent_id_reuse() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_top_level_id_collision() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_sibling_id_collision() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_empty_id() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_unknown_state_group() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_duplicate_state() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_multiple_initial_states() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_rejects_child_machine_duplicate_state_group() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
    ...
    #[test]
    fn static_contract_bundle_accepts_child_guard_descriptor_event_payload_read_from_catalog() {
        let mut bundle = sample_static_contract_bundle();
        let graph = bundle.machine_graphs.first_mut().unwrap();
        graph
            .event_catalog
        ...
        child.transitions[0].action = None;
        child.transitions[0].guard_descriptor = Some(MachineGuardDescriptor {
            guard_id: "bundle_child_payload_guard".to_string(),
            reads: vec![MachineGuardReadRef {
                source: MachineGuardReadSource::EventPayload,
                path: "symbol".to_string(),
            }],
            ...
            policy: None,
            explanation: Some(
                "bundle child guard reads a declared event payload field".to_string(),
            ),
        });
        risk.states[0].child_machine = Some(Box::new(child));
