...
use exchange_surface::{
    bar_interval_ms, endpoint_for_source, okx_endpoint_for_source, parse_okx_candles,
    parse_okx_ticker, ping_endpoint_for_source, provider_key_for_source,
};
...
pub(super) fn block_on_http<F, T>(future: F) -> Result<T>
where
...
#[cfg(target_os = "windows")]
pub(super) fn fetch_json_via_powershell(endpoint: String) -> Result<Value> {
