...
use qrpc_core_ir::{
    AgentPolicy, AgentPolicyKind, CoreMetadata, CoreSourceKind, CoreStrategyIr, CoreTimeInForce,
    ExecutionRule, ExecutionSizingKind, RebalanceSchedule, SignalKind, SignalRule,
};
...
fn sample_portfolio_with_symbol_position(
    ...
    portfolio.total_leverage =
        portfolio.total_gross_notional / (portfolio.cash_balance + portfolio.total_net_notional);
    portfolio
}

fn sample_portfolio_with_position(exchange: Exchange, qty: f64, mark_price: f64) -> PortfolioState {
...
fn portfolio_rebalance_agent_respects_daily_cadence() {
    ...
    core_ir.agent_policies[0].rebalance_schedule = Some(qrpc_core_ir::RebalanceSchedule::Every1d);
