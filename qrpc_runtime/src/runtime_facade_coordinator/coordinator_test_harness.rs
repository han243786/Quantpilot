...
use qrpc_core::{
    AgentConfig, DataKind, DataSourceConfig, DecisionStatus, Exchange, ExecutionPlan, IntentConfig,
    IntentKind, MarketType, NormalizedMarketData, PortfolioState, RiskConfig, RiskDecision,
    RuntimeEventType, RuntimeProtocolCoreConfig, Symbol,
};
...
impl IntentModuleProvider for NoopIntentModule {
    ...
    fn evaluate_intents(&self, _request: IntentEvaluationRequest<'_>) -> IntentEvaluationOutput {
        IntentEvaluationOutput {
            signals: Vec::new(),
            events: Vec::new(),
        }
...
impl ExecutionPlanner for NoopExecutionModule {
    ...
    fn plan_execution(&self, _request: ExecutionPlanningRequest<'_>) -> ExecutionPlanningOutput {
        ...
        Ok(RiskCheckOutput {
            decisions: request
                ...
                .map(|decision| RiskDecision {
                    risk_decision_id: format!("reject-{}-{}", decision.decision_id, request.now_ms),
                    risk_id: request
                        .core_ir
                        .risk_policies
                        .first()
