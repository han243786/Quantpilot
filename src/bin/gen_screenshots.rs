...
fn save_backtest(label: &str, config: &RuntimeProtocolCoreConfig) {
    ...
    let dir = std::path::PathBuf::from("storage").join("backtests").join(label);
    fs::create_dir_all(&dir).unwrap();
    fs::write(dir.join("manifest.json"), serde_json::to_string_pretty(&manifest).unwrap()).unwrap();
    fs::write(dir.join("backtest_output.json"), serde_json::to_string_pretty(&output).unwrap()).unwrap();
    ...
    println!("{label}: steps={} trades={} return={:.2}% sharpe={:.2}",
        output.summary.step_count, output.summary.trade_count,
...
fn main() {
    ...
    // 策略A: 激进型,满仓,低费率
    save_backtest("sc_a_aggressive", &RuntimeProtocolCoreConfig {
        data_sources: vec![DataSourceConfig { data_id:"d1".into(),exchange:Exchange::Binance,symbol:Symbol::BtcUsdt,market_type:MarketType::Spot,kind:DataKind::KlineSeries,days:Some(300),interval:Some("1d".into()),ping_enabled:false,request_interval_ms:None,enabled:true }],
        intents: vec![IntentConfig { intent_id:"buy".into(),name:"Buy".into(),kind:IntentKind::LongTermBuy,input_data_ids:vec!["d1".into()],params:BTreeMap::new(),enabled:true }, IntentConfig { intent_id:"sell".into(),name:"Sell".into(),kind:IntentKind::LongTermSell,input_data_ids:vec!["d1".into()],params:BTreeMap::new(),enabled:true }],
        agents: vec![AgentConfig { agent_id:"a1".into(),name:"Agent".into(),input_intent_ids:vec!["buy".into(),"sell".into()],rebalance_symbols:vec![],rebalance_schedule:None,rebalance_allocation_kind:None,rebalance_rank_method:None,rebalance_score_normalize:None,rebalance_target_weights:vec![],params:BTreeMap::new(),enabled:true }],
        risks: vec![RiskConfig { risk_id:"r1".into(),name:"Risk".into(),observed_agent_ids:vec!["a1".into()],max_position_ratio:1.0,max_single_weight:None,max_concentration_ratio:None,max_symbol_net_exposure_ratio:None,max_portfolio_net_exposure_ratio:None,max_turnover:None,min_trade_weight:None,max_new_positions_per_rebalance:None,max_total_leverage:1.0,max_exchange_leverage:1.0,min_action_interval_ms:0,enabled:true }],
        initial_cash_balance:100_000.0,taker_fee_bps:5.0,default_slippage_bps:3.0,total_cost_buffer_bps:10.0,
    });

    // 策略B: 保守型,半仓,高费率
    save_backtest("sc_b_conservative", &RuntimeProtocolCoreConfig {
        data_sources: vec![DataSourceConfig { data_id:"d1".into(),exchange:Exchange::Binance,symbol:Symbol::BtcUsdt,market_type:MarketType::Spot,kind:DataKind::KlineSeries,days:Some(300),interval:Some("1d".into()),ping_enabled:false,request_interval_ms:None,enabled:true }],
        intents: vec![IntentConfig { intent_id:"buy".into(),name:"Buy".into(),kind:IntentKind::LongTermBuy,input_data_ids:vec!["d1".into()],params:BTreeMap::new(),enabled:true }, IntentConfig { intent_id:"sell".into(),name:"Sell".into(),kind:IntentKind::LongTermSell,input_data_ids:vec!["d1".into()],params:BTreeMap::new(),enabled:true }],
        agents: vec![AgentConfig { agent_id:"a1".into(),name:"Agent".into(),input_intent_ids:vec!["buy".into(),"sell".into()],rebalance_symbols:vec![],rebalance_schedule:None,rebalance_allocation_kind:None,rebalance_rank_method:None,rebalance_score_normalize:None,rebalance_target_weights:vec![],params:BTreeMap::new(),enabled:true }],
        risks: vec![RiskConfig { risk_id:"r1".into(),name:"Risk".into(),observed_agent_ids:vec!["a1".into()],max_position_ratio:0.3,max_single_weight:None,max_concentration_ratio:None,max_symbol_net_exposure_ratio:None,max_portfolio_net_exposure_ratio:None,max_turnover:None,min_trade_weight:None,max_new_positions_per_rebalance:None,max_total_leverage:1.0,max_exchange_leverage:1.0,min_action_interval_ms:500,enabled:true }],
        initial_cash_balance:100_000.0,taker_fee_bps:15.0,default_slippage_bps:8.0,total_cost_buffer_bps:25.0,
    });
