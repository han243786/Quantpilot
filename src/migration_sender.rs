...
pub async fn deploy_strategy(
    ...
    let graph_json = body.get("graph_json").cloned().ok_or_else(|| {
        (axum::http::StatusCode::BAD_REQUEST, "缺少 graph_json".to_string())
    })?;

    // 1. QS lowering → RuntimeProtocolCoreConfig
    let protocol = crate::compile_api::compile_runtime_protocol_via_qs(&graph_json)
        .map_err(|e| (axum::http::StatusCode::BAD_REQUEST, format!("编译失败: {}", e.1)))?;

    // 2. 编译 → Core IR
    let compiled = qrpc_compiler::compile_runtime_protocol_config(&protocol)
        .map_err(|e| (axum::http::StatusCode::BAD_REQUEST, format!("Core IR 编译失败: {}", e)))?;

    let name = graph_json.get("metadata")
        .and_then(|v| v.get("name")).and_then(|v| v.as_str())
        .unwrap_or("未命名策略");
    let params = BTreeMap::new();
    let graph_id = graph_json.get("metadata").and_then(|v| v.get("graph_id")).and_then(|v| v.as_str()).unwrap_or("unknown");
    let compile_id = format!("{}_{}", graph_id, compiled.config_hash.chars().take(8).collect::<String>());
    ...
    let strategy_id = send_to_executor(&pkg).await
        .map_err(|e| (axum::http::StatusCode::SERVICE_UNAVAILABLE, format!("{}", e)))?;
...
pub fn build_migration_package(
    ...
    let protocol_hash = qrpc_core::canonical_json_sha256_digest(
        &serde_json::to_value(core_ir).unwrap_or_default()
    ).map(|d| d.value).unwrap_or_default();

    // v3.3.0: 签名覆盖 graph_json + params_snapshot 哈希
    let graph_hash = qrpc_core::canonical_json_sha256_digest(graph_json)
        .map(|d| d.value).unwrap_or_default();
    let params_hash = qrpc_core::canonical_json_sha256_digest(
...
pub async fn send_to_executor(pkg: &MigrationPackage) -> Result<String> {
    ...
    tokio::task::spawn_blocking(move || {
        let executor_url = std::env::var("QUANTPILOT_EXECUTOR_URL").unwrap_or_else(|_| "http://127.0.0.1:3001".into());
