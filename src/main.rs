...
async fn run_api_server() -> anyhow::Result<()> {
    ...
    // v2.3.0: 后台任务 — 每 tick 包含 try-catch 防止单次 panic 导致整个循环终止
    // 注: tokio spawn 本身会捕获子任务 panic, 无需额外 catch_unwind
    let bg_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            // v2.1.0: 每日备份 Permanent 级数据 (1440 ticks = 24h)
            if tick.is_multiple_of(1440) {
                backup::backup_permanent_storage().await;
            }
            // v2.1.0: TTL清理频率提升至每10分钟+条目数上限
            storage_lifecycle::startup_storage_cleanup(std::path::Path::new("storage"));
            if tick.is_multiple_of(10) {
                let cutoff = current_time_ms().saturating_sub(24 * 3600 * 1000);
                const MAX_CACHED_RECORDS: usize = 500;
                let mut runs = expiry_state.runs.write().await;
                runs.retain(|_, r| r.created_at_ms > cutoff);
                if runs.len() > MAX_CACHED_RECORDS {
                    let excess = runs.len() - MAX_CACHED_RECORDS;
                    let to_remove: Vec<_> = runs.iter().take(excess).map(|(k, _)| k.clone()).collect();
                    for k in to_remove { runs.remove(&k); }
                }
                drop(runs);
                let mut backtests = expiry_state.backtests.write().await;
                backtests.retain(|_, r| r.created_at_ms > cutoff);
                if backtests.len() > MAX_CACHED_RECORDS {
                    let excess = backtests.len() - MAX_CACHED_RECORDS;
                    let to_remove: Vec<_> = backtests.iter().take(excess).map(|(k, _)| k.clone()).collect();
                    for k in to_remove { backtests.remove(&k); }
                }
                drop(backtests);
                let mut experiments = expiry_state.experiments.write().await;
                experiments.retain(|_, r| r.created_at_ms > cutoff);
                if experiments.len() > MAX_CACHED_RECORDS {
                    let excess = experiments.len() - MAX_CACHED_RECORDS;
                    let to_remove: Vec<_> = experiments.iter().take(excess).map(|(k, _)| k.clone()).collect();
                    for k in to_remove { experiments.remove(&k); }
                }
            }
        }
    });
    ...
    for approval in approvals.values_mut() {
        if (approval.review_state == RuntimeApprovalReviewState::Pending
            ...
            && now_ms > approval.expires_at_ms {
                ...
                // v2.0.1: 使用 tokio::fs 异步持久化, 避免 spawn_blocking 写入丢失
