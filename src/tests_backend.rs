    ...
    fn compare_metrics_summaries_reports_field_level_differences() {
        let compared = compare_metrics_summaries(
            Some(qrpc_core::BacktestSummary {
                ...
                risk_adjusted: Default::default(),
                trade_analysis: Default::default(),
                drawdown_analysis: qrpc_core::BacktestDrawdownAnalysis { max_drawdown_ratio: 0.02, ..Default::default() },
                benchmark_comparison: None, skewness: 0.0, kurtosis: 0.0,
            }),
            Some(qrpc_core::BacktestSummary {
                step_count: 10,
    ...
    fn formal_compile_error_golden_view(value: &serde_json::Value) -> serde_json::Value {
        ...
        let detail_code = first_detail.and_then(|d| d.get("code")).cloned().unwrap_or(Value::Null);
        let detail_msg = first_detail.and_then(|d| d.get("message")).cloned().unwrap_or(Value::Null);
        let detail_reason = first_detail.and_then(|d| d.get("reason")).cloned().unwrap_or(Value::Null);
        let detail_span = first_detail.and_then(|d| d.get("span_label")).cloned().unwrap_or(Value::Null);
