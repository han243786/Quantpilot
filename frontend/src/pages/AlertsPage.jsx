...
export default function AlertsPage() {
  ...
  return (
    <div className="qp-page">
      ...
      {loading && <div className="qp-loading">加载告警数据...</div>}
      ...
      {!loading && !error && (
        <>
          ...
          <h3>活跃告警 ({data.firings.length})</h3>
          {data.firings.length === 0 && (
            <div className="qp-empty">暂无活跃告警，系统运行正常</div>
          )}
          {data.firings.map((f) => (
            <div className="qp-card qp-fade-in" key={f.firing_id} role="listitem">
              ...
              <div className="qp-card__meta">
                ...
                {f.acknowledged_by && <span>确认人: {f.acknowledged_by}</span>}
              </div>
              <div className="qp-card__body">{f.detail}</div>
            </div>
          ))}

          <h3>告警规则 ({data.rules.length})</h3>
          {data.rules
            .filter((r) => r.enabled)
            .map((r) => (
