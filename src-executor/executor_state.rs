...
use qrpc_core_ir::{CoreMetadata, CoreSourceKind, ExecutionRule, ExecutionSizingKind, CoreTimeInForce};
...
pub struct KlineBar {
    pub open_time_ms: u64, pub close_time_ms: u64,
    pub open: f64, pub high: f64, pub low: f64, pub close: f64, pub volume: f64,
}

pub struct RingBuffer {
    pub bars: VecDeque<KlineBar>, pub capacity: usize,
}

impl RingBuffer {
    pub fn new(capacity: usize) -> Self {
        Self { bars: VecDeque::with_capacity(capacity), capacity: capacity.max(1) }
    }
    pub fn push(&mut self, bar: KlineBar) {
        if self.bars.len() >= self.capacity { self.bars.pop_front(); }
        self.bars.push_back(bar);
    }
    pub fn latest(&self) -> Option<&KlineBar> { self.bars.back() }
}

#[derive(Debug, Clone)]
pub struct ActiveStrategy {
    pub strategy_id: String, pub name: String,
    pub core_ir: CoreStrategyIr, pub graph_json: serde_json::Value,
...
pub enum StrategyStatus { Loaded, Running, Stopped, Error(String) }
...
pub struct TriggerEvent {
    pub strategy_id: String, pub trigger_type: String,
    pub node_id: String, pub strength: f64, pub occurred_at_ms: u64,
}
...
impl ExecutorState {
    ...
    pub fn current_mode(&self) -> ExecutionMode {
        self.global_mode.read().unwrap_or_else(|e| e.into_inner()).clone()
    }
    ...
    pub fn register(&self, s: ActiveStrategy) -> anyhow::Result<()> {
        let mut strategies = self.strategies.write().map_err(|e| anyhow::anyhow!("锁: {}", e))?;
        // v3.0.2 B-1: 策略数上限保护
        if strategies.len() >= Self::MAX_STRATEGIES && !strategies.contains_key(&s.strategy_id) {
            anyhow::bail!("策略数已达上限 ({}), 请先停止或删除旧策略", Self::MAX_STRATEGIES);
        }
    ...
    pub fn persist(&self) -> anyhow::Result<()> {
        let strategies = self.strategies.read().map_err(|e| anyhow::anyhow!("锁: {}", e))?;
        let simplified: BTreeMap<String, serde_json::Value> = strategies.iter().map(|(id, s)| {
        ...
        let tmp_path = dir.join(".executor-state.tmp");
    ...
    pub fn load_from_file(path: &str) -> anyhow::Result<Arc<Self>> {
        ...
        for (id, val) in data {
            ...
            let params = val.get("params").and_then(|v| v.as_object()).map(|o| {
                o.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
            }).unwrap_or_default();
            strategies.insert(id.clone(), ActiveStrategy {
...
mod tests {
    ...
    fn ring_buffer_capacity_enforced() {
        ...
        for i in 1..=4 {
            buf.push(KlineBar { open_time_ms: i, close_time_ms: i+1, open: i as f64, high: (i+1) as f64, low: i as f64, close: (i as f64)+0.5, volume: 10.0 });
        }
    ...
    fn ring_buffer_capacity_one() {
        ...
        buf.push(KlineBar { open_time_ms: 1, close_time_ms: 2, open: 1.0, high: 2.0, low: 1.0, close: 1.5, volume: 10.0 });
        assert_eq!(buf.bars.len(), 1);
        buf.push(KlineBar { open_time_ms: 2, close_time_ms: 3, open: 2.0, high: 3.0, low: 2.0, close: 2.5, volume: 10.0 });
        assert_eq!(buf.bars.len(), 1);
        assert_eq!(buf.bars[0].open_time_ms, 2);
    }
