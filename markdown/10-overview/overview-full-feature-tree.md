...
当前架构补充: BE-001Z-01 已确认 `record_lifecycle` 值得作为下一候选，但当前 `no code movement`，不得直接移动 list/detail/save/discard。
当前最新递归点: 下一步只能进入 BE-001AA-01 `runtime.backtest.experiment_sweep.record_lifecycle` 单子叶等价基线。
