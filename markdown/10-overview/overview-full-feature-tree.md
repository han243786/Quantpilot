...
| `/api/runtime/backtest/*` | 回测 | `src/backend/runtime/routes/backtest.rs`、`src/runtime/backtest/execution_start.rs`、`src/runtime/backtest.rs`、`src/backtest_compare.rs` |
...
- `src/runtime/backtest/execution_start.rs` — backtest 创建路径 handler、legacy/v4 execution helper、v4 graph/symbol/event helper、v4 projection candidate 和 transient record 写入; 改 backtest start 执行入口时改这里 🆕 v4.16.0
