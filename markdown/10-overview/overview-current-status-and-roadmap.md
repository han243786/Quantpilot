...
- v4.16.0 模块化抽离已完成 system 入口抽离: `run_server`、`run_api_server` 和启动期 helper 归入 `system.entry.backend_process`，`quantpilot::run_server` 兼容入口保持不变；system 经验已回填为后续抽离准则，10 个叶子的等价基线已铺好，递归模块化流程已明确
...
| v4.16.0 | MINOR architecture / governance | 模块化抽离第一波，已完成 system 试水、经验回填、10 叶等价基线和递归模块化流程，后续六大顶层模块按同一算法推进 | 控制面落地中 |
