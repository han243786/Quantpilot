...
| 执行配置 | 用户能区分本地仿真、交易所演示盘、真实提交能力，避免静默降级。 | 主应用运行模式仅 `paper`；v4 运行使用 `PaperSimulated`；执行端支持 `PaperSimulated` 和 OKX demo 边界的 `PaperActual`；`live_execution_allowed=false`。 | 当前已实现 | `frontend/src/capabilities/supportMatrix.js`、`src/runtime/run.rs`、`src/runtime/event_stream.rs`、`src-executor/executor_state.rs`、`src-executor/main.rs`。 |
