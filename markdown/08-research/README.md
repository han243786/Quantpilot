# 研究文档

研究文档仅作为参考资料保留。
它们不是活跃的实现真实数据源，不是当前的发布门禁，也不是规划队列。

当研究措辞与活跃产品或发布状态冲突时，以活跃文档为准：

- [当前状态与发布状态](../overview/overview-current-status-and-roadmap.md)
- [首次发布就绪](../implementation/planning/implementation-first-release-readiness.md)
- [支持矩阵](../implementation/governance/implementation-support-matrix.md)
- [测试层期望](../implementation/runtime/implementation-test-layer-expectations.md)

不要将研究笔记中的旧路线图、优先级或能力声明复制回活跃规划中，除非已根据活跃合约文档重新验证。

- [深度研究报告](./deep-research-report.md)
- [架构重构蓝图](./research-architecture-refactor-blueprint.md)
- [回测工件协议研究](./research-backtest-artifact-protocol.md)
- [前后端 E2E 计划研究](./research-frontend-backend-e2e-plan.md)
- [QuantScript 类型化 HIR 诊断研究](./research-quantscript-typed-hir-diagnostics.md)
- [价差自定义插件排序研究](./research-spread-custom-plugin-sequencing.md)
