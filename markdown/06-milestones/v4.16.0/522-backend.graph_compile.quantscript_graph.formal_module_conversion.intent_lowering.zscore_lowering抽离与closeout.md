# v4.16.0 backend.graph_compile.quantscript_graph.formal_module_conversion.intent_lowering.zscore_lowering extract closeout

> Version type: MINOR architecture / governance
> Execution tier: lightweight
> Batch: BE-001GJ-02
> Baseline plan: `521-backend.graph_compile.quantscript_graph.formal_module_conversion.intent_lowering.zscore_lowering等价基线与抽离方案.md`
> Target child: `backend.graph_compile.quantscript_graph.formal_module_conversion.intent_lowering.zscore_lowering`
> Stage: `extract_closeout`
> Movement: Rust branch extraction + single leaf closeout
> Speed protocol: `lightweight_two_step`
> Next step: BE-001GK-01 `intent_lowering` parent residual judgment
...
| --- | --- | --- |
| Flow matrix | BE-001GJ-02 extract + closeout | lightweight two-step stage 2 |
| Norm matrix | parent-child communication / leaf split gate / equivalence proof | hard rule execution |
| Guidance matrix | `root.backend.graph_compile.quantscript_graph.formal_module_conversion.intent_lowering.zscore_lowering` | child white-box node closed |
| Module tree | `intent_lowering -> zscore_lowering` | new one-way child edge |
...
window default 20
entry_z default 2.0
entry_z.abs() used in negative threshold guard
...
| --- | --- | --- |
| white_box_boundary_named | pass | `zscore_lowering` now has a named child file and helper. |
| parent_child_communication_kept | pass | Parent resolves shared context and calls child once; child does not call siblings. |
| equivalence_baseline_freezable | pass | BE-001GJ-01 froze defaults, output lines, and helper surface. |
...
| local_proof_missing | no | Current leaf can be proven by compile/check and formal zscore tests. |
...
```text
stop_split_true
zscore_lowering actual_extraction_done
zscore_lowering closeout_done
...
```text
intent_lowering -> zscore_lowering
```
