# v4.16.0 runtime.mutation.ai_proposal.event_lifecycle_import_pass 抽离方案

> 版本类型: MINOR architecture / governance
> 执行档位: 标准
> 批次: BE-001EV-02
> 基线: `430-runtime.mutation.ai_proposal.event_lifecycle_import_pass单子叶等价基线.md`
...
> 下一步: BE-001EV-03 `runtime.mutation.ai_proposal.event_lifecycle_import_pass` 实际抽离记录
...
| 矩阵 | 影响节点 | 变更类型 |
| --- | --- | --- |
| 流程矩阵 | BE-001EV-02 `runtime.mutation.ai_proposal.event_lifecycle_import_pass` 抽离方案 | 方案优化 |
| 规范矩阵 | single-file import rewrite / event lifecycle freeze / no release transition | 固定抽离边界 |
| 引导矩阵 | `root.backend.runtime.runtime.parent_import_bridge.runtime.mutation.ai_proposal.event_lifecycle_import_pass` | event lifecycle import pocket |
| 模块树 | `runtime.mutation.ai_proposal.event_lifecycle_import_pass` | 抽离方案 |

---

## 方案结论

BE-001EV-03 只能改写 `src/runtime/mutation/ai_proposal/event_lifecycle.rs` 顶部 import，禁止改函数体、可见性、event type、reason code、payload schema、severity、summary、lifecycle sequence、persist order 或 sibling owner。
...
event_lifecycle_import_pass plan_frozen
single_file_event_lifecycle_import_rewrite
next_step: BE-001EV-03 extraction record
```
...
`src/runtime/mod.rs` 与 `src/runtime/mutation/ai_proposal.rs` 仍不在 BE-001EV-03 范围内。
...
1. 不改 Rust 代码。
2. 不执行 BE-001EV-03 实际 import rewrite。
...
AI 声称 BE-001EV-02 完成时，必须说明:

1. 本批只是 `no code movement` 抽离方案。
2. BE-001EV-03 只能改写 `src/runtime/mutation/ai_proposal/event_lifecycle.rs` 顶部 import。
3. 不得宣称 `event_lifecycle.rs` 已完成实际抽离。
...
1. `431-runtime.mutation.ai_proposal.event_lifecycle_import_pass抽离方案.md` 进入里程碑索引、模块树、全量树和治理门禁。
2. BE-001EV-03 的允许改动被限制为单文件 import rewrite。
3. BE-001EV-03 成为唯一下一步。
