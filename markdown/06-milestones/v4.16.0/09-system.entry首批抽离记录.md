...
> 执行档位: 重型。
> 判定: 以 `system.entry.backend_process` 作为 system 试水抽离；首批只移动启动 public 入口，不迁移 router、handler、runtime state 或 executor state。
> 后续: system 完成记录见 `10-system抽离完成记录.md`。

---

## 落地范围

首批完成 `system.entry.backend_process` 的第一刀代码抽离:
...
首批不移动 `run_api_server`，不移动 `build_app_router`，不移动 `new_app_state`，不改 HTTP route、response schema、存储路径、锁顺序或后台任务语义。后续 `10-system抽离完成记录.md` 已将 `run_api_server` 迁入 `system.entry.backend_process`。
...
| API 边界 | 首批时 `run_api_server`、`build_app_router` 未迁移；后续完成记录迁移 `run_api_server`，`build_app_router` 仍未迁移 |
...
首批之后继续拆 `system.entry.backend_process` 内部，必须先讨论以下决策:

1. 是否把 `run_api_server` 从 `src/lib.rs` 迁入 system 模块。结论见 `10-system抽离完成记录.md`。
2. 如果迁移 `run_api_server`，如何处理其对 `AppState`、后台任务、CORS、安全中间件和持久化预热的访问。结论见 `10-system抽离完成记录.md`。
