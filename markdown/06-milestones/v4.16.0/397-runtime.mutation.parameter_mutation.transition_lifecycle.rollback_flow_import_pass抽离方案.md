# v4.16.0 runtime.mutation.parameter_mutation.transition_lifecycle.rollback_flow_import_pass 抽离方案

> 版本类型: MINOR architecture / governance
> 执行档位: 标准
> 批次: BE-001EG-02
> 基线: `396-runtime.mutation.parameter_mutation.transition_lifecycle.rollback_flow_import_pass单子叶等价基线.md`
...
| 矩阵 | 影响节点 | 变更类型 |
| --- | --- | --- |
| 流程矩阵 | BE-001EG-02 `runtime.mutation.parameter_mutation.transition_lifecycle.rollback_flow_import_pass` 抽离方案 | 实施边界冻结 |
| 规范矩阵 | staged explicit import pass / single-file rewrite / parent-child communication | 改写约束冻结 |
| 引导矩阵 | `root.backend.runtime.runtime.parent_import_bridge.runtime.mutation.parameter_mutation.transition_lifecycle.rollback_flow_import_pass` | rollback flow import rewrite 指令 |
| 模块树 | `runtime.mutation.parameter_mutation.transition_lifecycle.rollback_flow_import_pass` | 固定下一步实际抽离范围 |
...
rollback_flow_import_pass plan_frozen
single_file_rollback_flow_import_rewrite
be_001eg_03_only_rewrite_rollback_flow_imports
...
BE-001EG-03 只允许改写一个文件的顶部 import:
...
如果 `cargo check` 暴露私有路径或 parent white-box import 差异，BE-001EG-03 只能最小修正 import 输入面并记录差异，不得改函数体。
...
1. `rollback_runtime_parameter_mutation` 名称、签名、`pub(crate)` 可见性不变。
2. capability guard、bad request details、original mutation record load、activated-only gate、rollback attempt metric、source run load、ledger lookup、boundary resolution、actor normalization、rollback id、governance projection、safe-window evaluation、lifecycle entry、event append 和 persistence 的调用顺序不变。
3. 只允许 `Activated` 状态进入 rollback。
4. unknown target version path 仍返回 `parameter_mutation_rollback_unknown_version`。
5. no-op target version path 仍返回 `parameter_mutation_rollback_noop`。
6. safe-window denied path 的 status、denied event、denied lifecycle、governance、append、metric、persist 和 `parameter_mutation_safe_window_denied` error response 不变。
7. rollback scheduled path 的 activation state、status、schedule event、lifecycle、metric、governance 和 event buffer 不变。
8. `next_cycle_start` immediate rollback path 的 activated time、status、rollback event、lifecycle、governance、metric 和 active parameter version 不变。
9. invalid resolved sequence path 的 `RollbackFailed` semantics 不变。
10. `append_parameter_mutation_events_to_run` 仍先于 `persist_runtime_parameter_mutation_transition`。
11. 不启动发布过渡，不引入 sibling horizontal link。
...
如果实际统计与预期不一致，BE-001EG-03 必须停在记录阶段说明差异，不得顺手扩大改写范围。
...
BE-001EG-03 完成后必须回到单叶 closeout，判断本 import pocket 是否值得继续细拆。
...
AI 声称 BE-001EG-02 完成时，必须说明:

1. 本批是 `no code movement`。
2. 下一步只允许改写 `src/runtime/mutation/parameter_mutation/transition_lifecycle/rollback_flow.rs` 顶部 import。
3. 不得改函数体、可见性、parent facade、activation flow、snapshot side effect 或 sibling。
4. 当前 residual 仍为 total 15 / mutation 13 / parameter_mutation 3 / transition_lifecycle 2。
...
1. `397-runtime.mutation.parameter_mutation.transition_lifecycle.rollback_flow_import_pass抽离方案.md` 进入里程碑索引、模块树、全量树和治理门禁。
2. BE-001EG-03 的单文件 import rewrite 边界被固定。
3. 不恢复旧三叶暂停目标。
4. Rust / 治理 / 全量树门禁均通过。
