...
**允许调用的子模块**:
`backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`src/runtime/mod.rs`、`src/runtime/run.rs`、`src/runtime/backtest.rs`、`src/runtime/mutation.rs`、`src/backtest_compare.rs` 中的 `pub(crate)` route targets。真实 run/backtest/mutation/report/experiment 子域仍留在 `src/runtime/`，后续若继续拆分必须另起单子叶等价基线。
...
**后续递归队列**:
后续 sibling 队列已由 BE-001AA-01 `runtime.backtest.experiment_sweep.record_lifecycle` 单子叶等价基线承接，BE-001AA-02 已建立抽离方案，下一步只能进入 BE-001AA-03 实际抽离记录。不得继续细拆 replay，也不得顺手迁移 query normalization、response mapping、schema、metrics、record lookup、state/persistence、artifact schema、frontend route 或发布过渡连接。
...
BE-001AA-02 已建立 `runtime.backtest.experiment_sweep.record_lifecycle` 抽离方案。下一批只能进入 BE-001AA-03 实际抽离记录，按计划目标 src/runtime/backtest/record_lifecycle.rs 迁移四个 lifecycle handler，并通过父级 `mod record_lifecycle;` 与受控 `pub(crate) use record_lifecycle::{...};` 保持兼容出口；当前仍不得提前创建物理子文件或移动代码。
...
**后续队列**:
下一步只能进入 BE-001AA-03 `runtime.backtest.experiment_sweep.record_lifecycle` 实际抽离记录。实际抽离阶段必须按 BE-001AA-02 的目标物理文件名、父级私有可见性、re-export 方式、focused test 和 rollback 点执行；不得顺手移动代码、删除 drained parent include、迁移 route facade、execution_start、record_store、replay、compare、state/persistence、response mapping、schema、frontend caller 或发布过渡连接，也不得宣称父叶最终完成。

**幻觉检查点**:
AI 声称 `runtime.backtest.experiment_sweep` 已完成 BE-001AA-01 时，必须说明本批次只是 `record_lifecycle` 单子叶等价基线，`no code movement`；`parameter_grid` 与 `start_orchestration` 已 closeout 并设置 `stop_split: true`，但父叶仍为 `stop_split: false`。不得宣称 `record_lifecycle` 已抽离、route facade、record_store、replay、compare、artifact schema owner、state owner、persistence owner、frontend caller、发布过渡、整理或重构已经完成。
...
**后续队列**:
本叶已完成 BE-001W-04 closeout，不继续细拆，并已交回父叶完成 BE-001X-01 与 BE-001Z-01 残余判断；BE-001Y-04 已完成 `runtime.backtest.experiment_sweep.start_orchestration` 单叶 closeout 并设置 `stop_split: true`；BE-001AA-01 已建立 `record_lifecycle` 单子叶等价基线，BE-001AA-02 已建立抽离方案。下一步只能进入 BE-001AA-03 实际抽离记录；不得直接修改 schema、改 `MAX_EXPERIMENT_VARIANTS`、删除 drained parent include、继续细拆 axis normalization/variant expansion 或宣称发布过渡。

**幻觉检查点**:
AI 声称 `runtime.backtest.experiment_sweep.parameter_grid` 已完成时，必须说明只完成 3 个 helper 的抽离与 closeout，并设置 `stop_split: true`；BE-001Y-04 已完成 `start_orchestration` closeout，BE-001Z-01 已完成第二轮父叶残余判断，但父叶仍未最终完成。不得宣称 route aggregate、execution_start、schema、state、persistence、response mapping、audit、frontend caller、发布版本过渡、整理和重构均已完成。
...
**父模块**: `runtime.backtest.experiment_sweep`
**状态**: v4.16 BE-001Y-04 单叶 closeout 已完成并设置 `stop_split: true`。`start_backtest_experiment` 已迁入 `src/runtime/backtest/start_orchestration.rs` 并确认等价；BE-001Z-01 已完成 `runtime.backtest.experiment_sweep` 第二轮父叶残余判断；BE-001AA-01 已建立 `record_lifecycle` 单子叶等价基线，BE-001AA-02 已建立抽离方案，后续只能进入 BE-001AA-03 实际抽离记录。
...
**后续队列**:
下一步只能进入 BE-001AA-03 `runtime.backtest.experiment_sweep.record_lifecycle` 实际抽离记录。不得把 `list_experiments`、`get_experiment_detail`、`save_experiment_record`、`discard_experiment_record` 的 record lifecycle 混入本子叶，也不得在方案前提前移动代码。

**幻觉检查点**:
AI 声称 `runtime.backtest.experiment_sweep.start_orchestration` 已 closeout 时，必须说明只完成 `start_backtest_experiment` 的抽离与等价 closeout，并设置 `stop_split: true`；BE-001Z-01 只完成父叶残余判断，下一候选才是 record lifecycle。不得宣称 record lifecycle、route registration、schema、state、persistence、response mapping、audit、frontend caller、发布过渡、整理或重构已经完成，也不得宣称父叶最终完成。
...
**父模块**: `runtime.backtest.experiment_sweep`
**状态**: v4.16 BE-001AA-01 单子叶等价基线已建立，BE-001AA-02 抽离方案已建立，当前仍是 `no code movement`。本计划节点冻结 `list_experiments`、`get_experiment_detail`、`save_experiment_record`、`discard_experiment_record` 的 record lifecycle 边界；下一步只能进入 BE-001AA-03 实际抽离记录。计划目标物理子文件为 src/runtime/backtest/record_lifecycle.rs，但该文件尚未创建。
...
**后续队列**:
下一步只能进入 BE-001AA-03 `runtime.backtest.experiment_sweep.record_lifecycle` 实际抽离记录。实际抽离阶段必须按 BE-001AA-02 确认的目标物理文件名、父级私有可见性、re-export 方式、focused test 和 rollback 点执行；不得直接移动代码，也不得混入 parameter_grid、start_orchestration、route registration、schema、persistence owner、response mapping owner、audit owner、frontend caller 或发布过渡连接。

**幻觉检查点**:
AI 声称 `runtime.backtest.experiment_sweep.record_lifecycle` 已有抽离方案时，必须说明本批是 `no code movement`，只确认 BE-001AA-03 的移动边界。不得宣称 record lifecycle 已抽离、物理子文件已存在、route registration、schema、state、persistence、response mapping、audit、frontend caller、发布过渡、整理或重构已经完成。
