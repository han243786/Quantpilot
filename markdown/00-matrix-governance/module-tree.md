...
**父模块**: `backend.runtime`
**状态**: v4.16 BE-001BD-01 `runtime.mutation.ai_proposal.event_lifecycle` 单子叶等价基线已建立；`runtime.mutation.ai_proposal.static_check` 与 `runtime.mutation.ai_proposal.source_governance_identity` 均已完成 closeout 并设置 `stop_split: true`，但父叶仍承接 event_lifecycle、record_query、approval_review、approval_persistence、sandbox_trigger 和 status_transition 等稳定职责，因此父叶保持 `stop_split: false`。下一步只能进入 BE-001BD-02 event_lifecycle 抽离方案。`AppState`、schema owner、frontend caller、runtime persistence owner、route facade 和 release transition guard 均未改变。
...
**细分价值判断**:
本父叶已完成 BE-001BD-01 `event_lifecycle` 单子叶等价基线，仍设置 `stop_split: false`。`static_check` 与 `source_governance_identity` 均已 closeout 且 `stop_split: true`；下一步只能进入 BE-001BD-02 抽离方案。父叶剩余 event_lifecycle、record_query、approval_review、approval_persistence、sandbox_trigger 与 status_transition。

**幻觉检查点**:
AI 声称 `runtime.mutation.ai_proposal` 已推进至 BE-001BD-01 时，必须说明当前只是建立 `event_lifecycle` 单子叶等价基线，仍为 `no code movement`，下一步只能进入 BE-001BD-02 抽离方案。不得宣称 event_lifecycle 已抽离、`runtime.mutation.ai_proposal` 父级已完成、approval review 已单独 closeout、AppState/schema/frontend caller 已改变、sandbox owner 已迁移、release transition 已启动、整理或重构已经完成。
...
**父模块**: `runtime.mutation.ai_proposal`
**状态**: v4.16 BE-001BD-01 单子叶等价基线已建立；当前 `no code movement`，只冻结 event contract、event payload、lifecycle entry、transition persistence 与非目标边界。下一步只能进入 BE-001BD-02 抽离方案。目标 child 文件尚未创建，helper 尚未迁移，record query、approval review、approval persistence、sandbox trigger、AppState、schema owner、frontend caller、route facade 和 release transition guard 均未改变。
...
**父级通信规则**:
后续若创建 child，只能被父级 `runtime.mutation.ai_proposal` 调用。helper 若迁移必须保持 `pub(super)`，不得把 event builder、lifecycle entry 或 transition persistence 暴露给 sibling。ASCII guard: `release transition guard`。

**回归保护**:
`cargo fmt --check`；`cargo check -p quantpilot`；`cargo test --no-run`；`cargo test -p quantpilot --test api_ai_proposal`；`cargo test -p quantpilot --test api_mutation`；`cargo test -p quantpilot --test api_evidence_contract`；`cargo test -p quantpilot --test api_run`；`powershell -NoProfile -ExecutionPolicy Bypass -File tools\check-matrix-governance.ps1`；`powershell -NoProfile -ExecutionPolicy Bypass -File tools\check-full-feature-tree.ps1`；`powershell -NoProfile -ExecutionPolicy Bypass -File tools\check-utf8.ps1`；`git diff --check`。

**细分价值判断**:
本批只建立等价基线，不做实际抽离，也不判断内部是否继续细拆。后续必须先进入 BE-001BD-02 抽离方案；不得直接创建目标文件、移动 helper、迁移 record query / approval review / sandbox trigger 或启动 release transition。

**幻觉检查点**:
AI 声称 `runtime.mutation.ai_proposal.event_lifecycle` 已推进至 BE-001BD-01 时，必须说明当前只是等价基线且为 `no code movement`。不得宣称 helper 已迁移、目标文件已创建、approval review 已拆分、AppState/schema/frontend caller 已改变、release transition 已启动、整理或重构已经完成。
