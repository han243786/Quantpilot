...
**父模块**: `backend.runtime`
**最新状态补充**: BE-001BW-01 已建立 `backend.runtime.routes.event_stream` 单子叶等价基线。当前 `backend.runtime.routes` 通过 `backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation` 与 `backend.runtime.routes.experiment` 委托五个 route child；event_stream 与 report_ops route 仍由父级直接注册，因此父叶保持 `stop_split: false`，下一步只能进入 BE-001BW-02 `backend.runtime.routes.event_stream` 抽离方案。
...
**父叶残余判断**:
BE-001BW-01 已建立 `backend.runtime.routes.event_stream` 单子叶等价基线；`backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation` 与 `backend.runtime.routes.experiment` 五个 route child 均由父级委托。event_stream 与 report_ops route 仍由父级直接注册，因此继续保持 `stop_split: false`。下一步只能进入 BE-001BW-02 `backend.runtime.routes.event_stream` 抽离方案，不得迁移 handler、AppState、schema、frontend caller、runtime persistence owner 或发布过渡连接。

**细分价值判断**:
`backend.runtime.routes.experiment` 已完成 BE-001BS-04 单叶 closeout 并设置 `stop_split: true`；继续拆成 sweep/list/save/detail/discard 微 facade 不会形成新的稳定 owner。BE-001BU-04 已确认 `backend.runtime.routes.evidence` 也设置 `stop_split: true`，继续拆 health/cleanup 微 facade 不会形成新的稳定 owner。BE-001BW-01 已为 `backend.runtime.routes.event_stream` 建立基线；下一步只允许进入 event_stream route facade 抽离方案。`backend.runtime.routes.report_ops` 保留为后续候选。

**幻觉检查点**:
AI 声称 `backend.runtime.routes` 已推进至 BE-001BW-01 时，必须说明父叶仍是 `stop_split: false`，event_stream 只是完成等价基线，event_stream/report_ops route 尚未迁移，handler/AppState/schema/frontend caller/runtime persistence owner 均未改变。不得宣称发布过渡已启动、整理或重构已经完成。
...
**下一步**:
只能进入 BE-001BW-02 `backend.runtime.routes.event_stream` 抽离方案；不得直接创建 route child file，不得迁移 `stream_run_events` handler，不得改变 SSE frame/keepalive、`AppState`、schema owner、frontend caller、runtime persistence owner 或 release transition guard。

**幻觉检查点**:
AI 声称 `backend.runtime.routes.event_stream` 已推进至 BE-001BW-01 时，必须说明只是等价基线，route registration 仍在父级，planned route child file 尚未创建，handler 与 state/persistence owner 均未改变。不得宣称 event stream route 已迁移、report_ops 已处理、`backend.runtime.routes` 父叶完成、发布过渡已启动、整理或重构已经完成。
