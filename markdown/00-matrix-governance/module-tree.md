...
**父子通信规则**:
`activation_flow` 只能作为 `transition_lifecycle` 的 child 被父级管理。后续若实际抽离，route facade 和 `src/runtime/mutation/parameter_mutation.rs` 仍只能经父级 `transition_lifecycle` 的受控出口调用，不得让 AI proposal、approval review、frontend caller 或发布过渡连接直接依赖本叶。
**允许调用的子模块**:
