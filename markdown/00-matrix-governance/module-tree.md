...
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.event_usage_validation.event_reference_resolution baseline_frozen: true`; BE-001UQ-02 may create a nested event reference resolution child helper and move only nested machine collection, transition/action/edge event reference collection, empty action-emits error reporting, and a small resolution result. Catalog presence checks, local catalog validation, event spec map construction, undeclared event checks, party validation, event catalog DTOs, graph validation orchestration, DAG validation, risk-plane validation, and tests remain outside this child.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.event_usage_validation.event_reference_resolution extracted: true`; `qrpc_core_ir/src/v4/machine_graph_contract/static_validation/event_usage_validation/event_reference_resolution.rs` now owns nested machine discovery, event reference collection, empty action-emits error reporting, and the resolution result. Parent `qrpc_core_ir/src/v4/machine_graph_contract/static_validation/event_usage_validation.rs` retains catalog presence checks, local catalog validation, event spec map construction, undeclared event checks, party validation dispatch, and orchestration.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.event_usage_validation.event_reference_resolution stop_split: true`; this leaf remains one coherent event reference resolution helper and result structure. Next parent step is `event_usage_validation` single leaf closeout.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.event_usage_validation stop_split: true`; this parent remains a compact event usage orchestrator after closing `event_reference_resolution` and `event_party_validation`. Next graph static validation residual candidate is `graph_acyclic_validation`.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation` selected `graph_acyclic_validation`; this child owns only `V4MachineGraphContract::validate_graph_acyclic` and local DFS/cycle path detection. Root graph validation orchestration, graph schema, edge identity/endpoint checks, event usage validation, risk-plane validation, event party helper, and traversal helpers remain outside this selected child.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.graph_acyclic_validation baseline_frozen: true`; BE-001UT-02 may create a nested graph acyclic validation child file and move only `V4MachineGraphContract::validate_graph_acyclic`. Root graph validation orchestration, schema/version checks, machine checks, edge identity/endpoint/self-loop checks, event usage validation, risk-plane validation, event party helper, graph schema, traversal helpers, bridge code, and tests remain outside this child.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.graph_acyclic_validation extracted: true`; `qrpc_core_ir/src/v4/machine_graph_contract/static_validation/graph_acyclic_validation.rs` now owns only `V4MachineGraphContract::validate_graph_acyclic`, adjacency construction, DFS cycle detection, and cycle path formatting. Parent `qrpc_core_ir/src/v4/machine_graph_contract/static_validation.rs` retains root graph validation orchestration, schema/version/machine/edge checks, event usage dispatch, risk-plane dispatch, and event party helper.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.graph_acyclic_validation stop_split: true`; this leaf remains one coherent topology validation helper. Next parent step is `graph_static_validation` single leaf closeout.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation continue_split: true`; the parent remains open because machine identity/delegation checks and edge identity/endpoint/self-loop checks are still separate failure planes. Next candidate is `machine_identity_validation`.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation` selected `machine_identity_validation`; this child owns machine ID required checks, duplicate top-level machine IDs, nested duplicate machine IDs, machine map construction, and per-machine static contract delegation. Schema/graph ID checks, edge identity/endpoint/self-loop checks, DAG validation, event usage validation, risk-plane validation, event party helper, graph schema, and traversal helpers remain outside this selected child.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.machine_identity_validation baseline_frozen: true`; BE-001UW-02 may create a nested machine identity validation child file and move only machine ID required checks, duplicate top-level/nested machine ID checks, machine map construction, per-machine static contract delegation, and a small result structure. Schema/graph ID checks, empty machine-list check, edge identity/endpoint/self-loop checks, DAG validation, event usage validation, risk-plane validation, event party helper, graph schema, traversal helpers, bridge code, and tests remain outside this child.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.machine_identity_validation extracted: true`; `qrpc_core_ir/src/v4/machine_graph_contract/static_validation/machine_identity_validation.rs` now owns machine ID required checks, duplicate top-level/nested machine ID checks, machine map construction, per-machine static contract delegation, and the `MachineIdentityValidation` result. Parent `qrpc_core_ir/src/v4/machine_graph_contract/static_validation.rs` retains schema/graph ID checks, empty machine-list check, edge identity/endpoint/self-loop checks, DAG/event/risk dispatch, and event party helper.
`root.contracts.core_ir.v4_contracts.machine_graph_contract.graph_static_validation.machine_identity_validation stop_split: true`; this leaf remains one coherent machine inventory validation helper. Next graph static validation residual candidate is `edge_identity_validation`.
