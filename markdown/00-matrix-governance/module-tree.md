...
**父模块**: `backend.runtime`
**最新状态补充**: BE-001BY-02 已建立 `backend.runtime.routes.report_ops` 抽离方案。当前 `backend.runtime.routes` 通过 `backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.event_stream`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation` 与 `backend.runtime.routes.experiment` 委托六个 route child；父叶仍直接持有 report_ops route，因此保持 `stop_split: false`，下一步只能进入 BE-001BY-03 `backend.runtime.routes.report_ops` 实际抽离。
...
**父叶残余判断**:
BE-001BY-02 已建立 `backend.runtime.routes.report_ops` 抽离方案；`backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.event_stream`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation` 与 `backend.runtime.routes.experiment` 六个 route child 均由父级委托且已完成当前递归 closeout。父叶仍直接持有 report_ops route，因此继续保持 `stop_split: false`。下一步只能进入 BE-001BY-03 `backend.runtime.routes.report_ops` 实际抽离，不得迁移 handler、schema、state owner、frontend caller、runtime persistence owner 或 release transition guard。

**细分价值判断**:
`backend.runtime.routes.experiment` 已完成 BE-001BS-04 单叶 closeout 并设置 `stop_split: true`；继续拆成 sweep/list/save/detail/discard 微 facade 不会形成新的稳定 owner。BE-001BU-04 已确认 `backend.runtime.routes.evidence` 也设置 `stop_split: true`，继续拆 health/cleanup 微 facade 不会形成新的稳定 owner。BE-001BW-04 已确认 `backend.runtime.routes.event_stream` 也设置 `stop_split: true`，继续拆 SSE 微 facade 不会形成新的稳定 owner。BE-001BY-02 已为 `backend.runtime.routes.report_ops` 建立抽离方案；下一步只允许进入 report_ops route facade 实际抽离。

**幻觉检查点**:
AI 声称 `backend.runtime.routes` 已推进至 BE-001BY-02 时，必须说明父叶仍是 `stop_split: false`，report_ops 只是完成抽离方案，report_ops route 尚未迁移，handler/AppState/schema/frontend caller/runtime persistence owner 均未改变。不得宣称发布过渡已启动、整理或重构已经完成。
...
**父模块**: `backend.runtime.routes`
**状态**: v4.16 BE-001BY-02 抽离方案已建立；当前 `no code movement`，下一步 BE-001BY-03 才允许创建 planned report_ops route child file，并以 `report_ops::register_runtime_report_routes(router)` 与 `report_ops::register_ops_routes(router)` 两个父级委托入口保持现有 route order。route registration 仍在 `src/backend/runtime/routes.rs`，handler owner 仍在 `src/runtime/mod.rs`，planned route child file 尚未创建。本节点 `stop_split: pending`，下一步只能进入 BE-001BY-03 实际抽离。
**真实文件**:
- `src/backend/runtime/routes.rs`
- `src/runtime/mod.rs`
- `src/runtime_persistence.rs`
...
| 方法 | 输入 | 输出 | 调用方 | 禁止事项 |
| --- | --- | --- | --- | --- |
| `backend.runtime.routes::register_routes` | Axum Router | runtime routes | `backend.runtime` | 当前仍直接注册 report_ops routes，不得迁移 handler owner |
| `report_ops::register_runtime_report_routes` | Axum Router | runtime report routes | `backend.runtime.routes` | 下一步只允许承接 `/api/runtime/reports*` routes，不得注册 v1 ops 或 experiment routes |
| `report_ops::register_ops_routes` | Axum Router | v1 ops routes | `backend.runtime.routes` | 下一步只允许承接 `/api/v1/*` report ops routes，不得注册 runtime report 或 experiment routes |
| `create_runtime_report` | user id、`AppState`、request | report record | route aggregate | 不得迁移 report persistence helper |
| `list_runtime_reports` | user id、`AppState`、pagination | paginated reports | route aggregate | 不得改变 sorting/pagination |
| `get_runtime_report_detail` | user id、`AppState`、report id | report record | route aggregate | 不得绕过 report store lookup |
| `export_runtime_report_artifact` | user id、`AppState`、report id | report artifact | route aggregate | 不得改变 artifact schema |
| `list_merge_records` | user id、`AppState` | merge records | route aggregate | 不得迁移 run state owner |
| `list_config_generations` | `AppState` | generation history | route aggregate | 不得迁移 config generation owner |
| `get_storage_health` | `AppState` | storage health | route aggregate | 不得迁移 storage lifecycle owner |
| `get_ops_daily_report` | user id、`AppState`、query | ops report | route aggregate | 不得迁移 report schema owner |
| `get_audit_weekly_report` | user id、`AppState`、query | audit report | route aggregate | 不得迁移 report schema owner |
| `get_research_monthly_report` | user id、`AppState`、query | research report | route aggregate | 不得迁移 report schema owner |
...
**下一步**:
只能进入 BE-001BY-03 `backend.runtime.routes.report_ops` 实际抽离；不得跳过 route facade 创建去迁移 report handler、`AppState`、schema owner、frontend caller、runtime persistence owner、storage lifecycle owner 或 release transition guard。

**幻觉检查点**:
AI 声称 `backend.runtime.routes.report_ops` 已推进至 BE-001BY-02 时，必须说明只是抽离方案，route registration 仍在父级，planned route child file 尚未创建，handler 与 state/persistence owner 均未改变。不得宣称 report_ops route 已迁移、`backend.runtime.routes` 父叶完成、发布过渡已启动、整理或重构已经完成。
