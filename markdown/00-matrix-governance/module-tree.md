...
**父叶残余判断**:
BE-001AD-01 已确认 `backend.runtime.routes.run`、`runtime.event_stream`、`backend.runtime.routes.backtest` 和 `runtime.backtest` 相关递归链路均已完成当前范围内 closeout；父叶仍保持 `stop_split: false`。BE-001AE-04 已完成 `backend.runtime.routes.mutation` route facade 单叶 closeout 并设置 `stop_split: true`；BE-001AF-04 已完成 `runtime.mutation.parameter_mutation` 单叶 closeout；BE-001AG-02 已建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案，下一步只能进入 BE-001AG-03 实际抽离，不得直接移动代码、AI proposal、approval、AppState、锁顺序、schema、frontend caller 或发布过渡连接。

**细分价值判断**:
`backend.runtime.routes.mutation` route facade 已完成 BE-001AE-04 单叶 closeout 并设置 `stop_split: true`，因为继续拆 58 行 route facade 只会制造无意义微文件。`src/runtime/mutation.rs` handler 域仍值得继续递归，`runtime.mutation.parameter_mutation` 已完成 BE-001AF-04 单叶 closeout，BE-001AG-02 已建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案；下一步只能进入 BE-001AG-03 实际抽离。`backend.runtime.routes.experiment`、`backend.runtime.routes.evidence` 和 `backend.runtime.routes.report_ops` 保留为后续候选，不得在本批顺手新建 route 子文件。
...
**状态**: v4.16 BE-001AE-04 单叶 closeout 已完成；`src/backend/runtime/routes/mutation.rs` 承接 mutation / AI proposal / approval route group，并由 `src/backend/runtime/routes.rs` 父级委托，route facade 等价且设置 `stop_split: true`。BE-001AF-04 已完成 `runtime.mutation.parameter_mutation` 单叶 closeout，BE-001AG-02 已建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案；`src/runtime/mod.rs` facade、`AppState`、`approval_records -> ai_proposals` 锁顺序、schema、frontend caller 和发布过渡均未改变。下一步只能进入 BE-001AG-03 实际抽离。
**真实文件**:
- `src/backend/runtime/routes.rs`
- `src/backend/runtime/routes/mutation.rs`
- `src/runtime/mutation.rs`
...
**细分价值判断**:
本节点已完成等价基线、抽离方案、route facade 最小物理抽离与单叶 closeout，route facade 设置 `stop_split: true`。继续把 facade 拆成 mutation routes、AI proposal routes 和 approval routes 会增加父级导入面，但不会形成新的稳定 owner；后续递归已转入 `src/runtime/mutation.rs` handler 域，`runtime.mutation.parameter_mutation` 已完成 BE-001AF-04 单叶 closeout，BE-001AG-02 已建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案，下一步只能进入 BE-001AG-03 实际抽离。`runtime.mutation.ai_proposal`、`runtime.mutation.approval_review` 值得后续排队，`runtime.mutation.shared_persistence_governance` 暂缓。
...
**状态**: v4.16 BE-001AF-04 单叶 closeout 已完成；第一轮抽离等价成立，但本叶设置 `stop_split: false`。BE-001AG-02 已建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案，当前 `no code movement`，下一步只能进入 BE-001AG-03 实际抽离。五个 parameter mutation public handler 与本叶私有 helper 已迁入 `src/runtime/mutation/parameter_mutation.rs`，父级 `src/runtime/mod.rs` 通过 `pub(crate) use mutation_parameter_mutation` 保持 route facade 调用面。AI proposal、approval review、AppState、schema、frontend caller、锁顺序、shared helper 和发布过渡连接未改变。
**真实文件**:
...
**细分价值判断**:
BE-001AF-04 已完成单叶 closeout，判定 `runtime.mutation.parameter_mutation` 暂不停止细拆，`stop_split: false`。BE-001AG-02 已建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案，原因是 activation / rollback lifecycle 共享 boundary、safe window、lifecycle entry、transition persistence、auto snapshot side effect 与 run event append，是本叶中风险和复杂度最高的事务流。下一步只能进入 BE-001AG-03 实际抽离；`proposal_record`、`safe_window_boundary` 和 `auto_snapshot_side_effect` 暂缓，等 transition lifecycle 抽离与 closeout 稳定后再判断。

**幻觉检查点**:
AI 声称 `runtime.mutation.parameter_mutation` 已推进至 BE-001AG-02 时，必须说明本批只建立 `runtime.mutation.parameter_mutation.transition_lifecycle` 抽离方案，仍为 `no code movement`，下一步只能进入 BE-001AG-03 实际抽离。不得宣称 transition lifecycle 已抽离、AI proposal/approval 已拆分、AppState/schema/frontend caller 已迁移、发布过渡已启动、整理或重构已经完成。
...
**路由入口**: `backend.runtime.routes.mutation`
**状态**: v4.16 BE-001AG-02 抽离方案已建立；当前 `no code movement`，planned target path 为 src/runtime/mutation/parameter_mutation/transition_lifecycle.rs，父级通过 `mod transition_lifecycle`、`pub(crate) use transition_lifecycle::{activate_runtime_parameter_mutation, rollback_runtime_parameter_mutation};` 和 `use transition_lifecycle::validate_runtime_parameter_mutation_boundary;` 维持 handler 与 boundary validation 出口。下一步只能进入 BE-001AG-03 实际抽离，不得混入 proposal create/list/detail、AI proposal、approval、AppState、schema、frontend caller 或发布过渡连接。
**真实文件**:
- `src/runtime/mutation/parameter_mutation.rs`
- `src/runtime/mod.rs`
- `src/runtime/mutation.rs`
...
**排除边界**:
不得迁移 `create_runtime_parameter_mutation`、`list_runtime_parameter_mutations`、`get_runtime_parameter_mutation_detail`、`runtime_parameter_mutation_record_id`、AI proposal、approval review、AppState、schema、frontend caller、route facade、测试 fixture 或发布过渡连接。BE-001AG-01 未创建 planned transition file；BE-001AG-02 已决定目标路径、re-export 面和迁移清单；BE-001AG-03 才能实际创建目标文件并移动代码。

**BE-001AG-02 抽离方案**:
下一批 BE-001AG-03 只允许创建 planned target path src/runtime/mutation/parameter_mutation/transition_lifecycle.rs，迁移 `activate_runtime_parameter_mutation`、`rollback_runtime_parameter_mutation`、`validate_runtime_parameter_mutation_boundary`、`resolve_runtime_parameter_mutation_boundary`、`evaluate_runtime_parameter_mutation_safe_window`、`runtime_parameter_mutation_rollback_record_id`、`mutation_lifecycle_entry`、`persist_runtime_parameter_mutation_transition` 和 `auto_snapshot_on_activation`。其中 `validate_runtime_parameter_mutation_boundary` 必须为 `pub(super)`，供父级 `create_runtime_parameter_mutation` 继续复用；其余 transition helper 保持 private。父级 `src/runtime/mutation/parameter_mutation.rs` 保留 `create_runtime_parameter_mutation`、`list_runtime_parameter_mutations`、`get_runtime_parameter_mutation_detail` 和 `runtime_parameter_mutation_record_id`。

**回归保护**:
`cargo fmt --check`；`cargo check -p quantpilot`；`cargo test --no-run`；`cargo test -p quantpilot --test api_mutation`；`cargo test -p quantpilot --test api_ai_proposal`；`cargo test -p quantpilot --test api_evidence_contract`；`cargo test -p quantpilot --test api_run`；`powershell -NoProfile -ExecutionPolicy Bypass -File tools\check-utf8.ps1`；`powershell -NoProfile -ExecutionPolicy Bypass -File tools\check-matrix-governance.ps1`；`powershell -NoProfile -ExecutionPolicy Bypass -File tools\check-full-feature-tree.ps1`；`git diff --check`。

**幻觉检查点**:
AI 声称 `runtime.mutation.parameter_mutation.transition_lifecycle` 已完成 BE-001AG-02 时，必须说明本批只建立抽离方案，仍为 `no code movement`，下一步只能进入 BE-001AG-03 实际抽离。不得宣称 transition lifecycle 已抽离、目标文件已创建、safe window 已迁移、auto snapshot owner 已迁移、AI proposal/approval 已拆分、AppState/schema/frontend caller 已改变或发布过渡已启动。
