...
**父模块**: `backend.runtime`
**最新状态补充**: BE-001BW-02 已建立 `backend.runtime.routes.event_stream` 抽离方案。当前 `backend.runtime.routes` 通过 `backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation` 与 `backend.runtime.routes.experiment` 委托五个 route child；event_stream 与 report_ops route 仍由父级直接注册，因此父叶保持 `stop_split: false`，下一步只能进入 BE-001BW-03 `backend.runtime.routes.event_stream` 实际抽离。
...
**允许调用的子模块**:
`backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation`、`backend.runtime.routes.experiment`、`src/runtime/mod.rs`、`src/runtime/run.rs`、`src/runtime/backtest.rs`、`src/runtime/mutation.rs`、`src/backtest_compare.rs` 中的 `pub(crate)` route targets。真实 run/backtest/mutation/report/experiment/evidence 子域仍留在 `src/runtime/`，后续若继续拆分必须另起单子叶等价基线。
...
**父叶残余判断**:
BE-001BW-02 已建立 `backend.runtime.routes.event_stream` 抽离方案；`backend.runtime.routes.run`、`backend.runtime.routes.backtest`、`backend.runtime.routes.evidence`、`backend.runtime.routes.mutation` 与 `backend.runtime.routes.experiment` 五个 route child 均由父级委托。event_stream 与 report_ops route 仍由父级直接注册，因此继续保持 `stop_split: false`。下一步只能进入 BE-001BW-03 `backend.runtime.routes.event_stream` 实际抽离，不得迁移 handler、AppState、schema、frontend caller、runtime persistence owner 或发布过渡连接。

**细分价值判断**:
`backend.runtime.routes.experiment` 已完成 BE-001BS-04 单叶 closeout 并设置 `stop_split: true`；继续拆成 sweep/list/save/detail/discard 微 facade 不会形成新的稳定 owner。BE-001BU-04 已确认 `backend.runtime.routes.evidence` 也设置 `stop_split: true`，继续拆 health/cleanup 微 facade 不会形成新的稳定 owner。BE-001BW-02 已为 `backend.runtime.routes.event_stream` 建立抽离方案；下一步只允许进入 event_stream route facade 实际抽离。`backend.runtime.routes.report_ops` 保留为后续候选。

**幻觉检查点**:
AI 声称 `backend.runtime.routes` 已推进至 BE-001BW-02 时，必须说明父叶仍是 `stop_split: false`，event_stream 只是完成抽离方案，event_stream/report_ops route 尚未迁移，handler/AppState/schema/frontend caller/runtime persistence owner 均未改变。不得宣称发布过渡已启动、整理或重构已经完成。
...
**父模块**: `backend.runtime.routes`
**状态**: v4.16 BE-001BW-02 抽离方案已建立；当前 `no code movement`，下一步 BE-001BW-03 才允许创建 route child facade 并迁移 `/api/runtime/runs/:run_id/events` GET route registration。route registration 仍在 `src/backend/runtime/routes.rs`，handler owner 仍在 `src/runtime/event_stream.rs`，planned route child file 尚未创建。本节点 `stop_split: pending`，下一步只能进入 BE-001BW-03 实际抽离。
**真实文件**:
- `src/backend/runtime/routes.rs`
- `src/runtime/event_stream.rs`
- `src/runtime/mod.rs`
...
| --- | --- | --- | --- | --- |
| `/api/runtime/runs/:run_id/events` | GET | `runtime_handlers::stream_run_events` | `src/backend/runtime/routes.rs` | 不得改变 route order 或 handler owner |

**关键 public 方法**:
| 方法 | 输入 | 输出 | 调用方 | 禁止事项 |
| --- | --- | --- | --- | --- |
| `backend.runtime.routes::register_routes` | Axum Router | runtime routes | `backend.runtime` | 当前仍直接注册 event stream route，不得迁移 handler owner |
...
**下一步**:
只能进入 BE-001BW-03 `backend.runtime.routes.event_stream` 实际抽离；只允许创建 route child facade、在父级声明 `pub mod event_stream`、迁移一条 route registration 并保持 run -> event_stream -> evidence -> mutation 顺序。不得迁移 `stream_run_events` handler，不得改变 SSE frame/keepalive、`AppState`、schema owner、frontend caller、runtime persistence owner 或 release transition guard。

**幻觉检查点**:
AI 声称 `backend.runtime.routes.event_stream` 已推进至 BE-001BW-02 时，必须说明只是抽离方案，route registration 仍在父级，planned route child file 尚未创建，handler 与 state/persistence owner 均未改变。不得宣称 event stream route 已迁移、report_ops 已处理、`backend.runtime.routes` 父叶完成、发布过渡已启动、整理或重构已经完成。
