...
BE-001Y-04 已完成 `runtime.backtest.experiment_sweep.start_orchestration` 单叶 closeout，确认 `start_backtest_experiment` 等价并设置 `stop_split: true`；下一步只能回到 `runtime.backtest.experiment_sweep` 父叶残余判断，默认进入 BE-001Z-01，不能从 start_orchestration 继续细拆或直接移动 record lifecycle、route/schema/state/persistence/response mapping/audit/frontend caller。
