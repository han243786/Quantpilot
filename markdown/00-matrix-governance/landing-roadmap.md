...
BE-001AE-04 已完成 `backend.runtime.routes.mutation` 单叶 closeout；route facade 等价并设置 `stop_split: true`。`src/runtime/mutation.rs` handler 域仍值得继续递归，默认下一候选为 `runtime.mutation.parameter_mutation`；下一步只能进入 BE-001AF-01 单子叶等价基线，不能移动 parameter mutation handler、AI proposal、approval review、shared persistence/governance helper、AppState、锁顺序、schema、frontend caller、report/evidence/experiment/ops 或发布过渡连接。
